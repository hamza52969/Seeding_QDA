...
LOG_PATH  = BASE_DIR / "pipeline6.log"
...
class HarvardDataversePipeline:
    ...
    def _extract_license_from_item(self, item: dict) -> str:
        """
        Pull licence from a Dataverse search result item, routing to the correct host.
        """
        doi = item.get("global_id", "")
        target_base = resolve_dataverse_base(doi) if doi else HARVARD_BASE
        ...
        if doi:
            ver = get_json(
                f"{target_base}/api/datasets/:persistentId/versions/:latest",
        ...
        if doi:
            ds = get_json(
                f"{target_base}/api/datasets/:persistentId/",
                params={"persistentId": doi},
            )
    ...
    def run(self, conn: sqlite3.Connection, dry_run: bool = False, test_mode: bool = False):
        ...
        for query in queries:
            ...
            for item in tqdm(items, desc=f"Harvard '{query}'", leave=False):
                ...
                # Dynamic routing for federated datasets
                ...
                proj_data = dict(
                    ...
                    repository_url             = target_base, # Use actual host URL
                ...
                # Fetch metadata from the CORRECT target base to get the files list
                ...
                version = ds_meta.get("latestVersion", {})
                file_list = version.get("files", [])
                dest_base = FILES_DIR / HARVARD_FOLDER / doi_slug
                
                for fmeta in file_list:
                    # Skip restricted files
                    if fmeta.get("restricted", False):
                        continue
                    ...
                    # Download from the correct target base without ?format=original
                    dl_url = f"{target_base}/api/access/datafile/{file_id}"
        ...
        for ext_query in qda_queries:
            ...
            for doi in parent_dois:
                ...
                if project_exists(conn, HARVARD_REPOID, doi_url):
                    continue

                proj_id = insert_project(conn, {
                    "query_string": f"file:{ext_query}",
                    "repository_id": HARVARD_REPOID,
                    "repository_url": target_base, 
                    "project_url": doi_url,
                    "title": str(title)[:500],
                    "download_date": now_ts(),
                    "download_repository_folder": HARVARD_FOLDER,
                ...
                for fmeta in file_list:
                    ...
                    dl_url = f"{target_base}/api/access/datafile/{real_file_id}"
